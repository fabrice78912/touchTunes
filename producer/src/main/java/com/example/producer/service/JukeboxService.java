...
public class JukeboxService {
    ...
    public Mono<ApiResponse<String>> activateJukebox(String serialNumber) {
        return repository.findBySerialNumber(serialNumber)
                .flatMap(jukeboxe -> Mono.just(ApiResponse.<String>builder()
                        ...
                        .build()))
                .switchIfEmpty(generateUniqueJukeboxId()
                        .flatMap(newId -> {
                            Jukeboxe newJukeboxe = Jukeboxe.builder()
                                    .serialNumber(serialNumber)
                                    .jukeboxId(newId)
                                    .build();
                            return repository.save(newJukeboxe)
                                    .map(saved -> ApiResponse.<String>builder()
                                            .timestamp(Instant.now())
    ...
    private Mono<String> generateUniqueJukeboxId() {
        ...
        return repository.findByJukeboxId(candidateId)
