...
public class JukeboxProducerService {
    ...
    public Mono<Object> sendEvent(JukeboxEvent event, java.lang.String path) {
        ...
        return jukeboxRepository.findByJukeboxId(jukeboxId)
                .flatMap(jukebox -> {
                    prepareEvent(event);
                    log.info("Après prepareEvent: eventId={}, timestamp={}, type={}",
                            event.getEventId(), event.getTimestamp(), event.getType());
                    ...
                    return Mono.create(sink ->
                            kafkaTemplate.send(TOPIC_NAME, partition, jukeboxId, event)
                                    .whenComplete((SendResult<java.lang.String, JukeboxEvent> result, Throwable ex) -> {
                                        if (ex == null) {
                                            java.lang.String formattedTime = DateTimeFormatter.ISO_LOCAL_DATE_TIME
                                            ...
                                            Map<java.lang.String, java.lang.String> dataMap = new ConcurrentHashMap<>();
                                            dataMap.put("eventId", event.getEventId());


                                            sink.success(ApiResponse.<Map>builder()
                                            ...
                                            log.error("❌ Kafka send error: eventId={} | jukeboxId={} | error={}",
                                                    event.getEventId(), jukeboxId, ex.getMessage());

                                            sink.success(ApiResponse.<java.lang.String>builder()
                                                    .timestamp(Instant.now())
                ...
                .switchIfEmpty(Mono.just(ApiResponse.<java.lang.String>builder()
                        .timestamp(Instant.now())
    ...
    public ApiResponse<?> sendEventMongo(JukeboxEvent event, java.lang.String path) {
        ...
        try {
            SendResult<java.lang.String, JukeboxEvent> result = kafkaTemplate.send(TOPIC_NAME, partition, jukeboxId, event).get();
